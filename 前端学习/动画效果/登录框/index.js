...
document.addEventListener('mousemove', (event) =>{
    ...
    const radius = Math.max(button.offsetWidth * 0.75,
        button.offsetHeight *0.75,100)
    ...
    const rx = oy / 2
    const ry = -ox / 2
    button.style.transition = `all 0.01s ease`
    button.style.transform = `translate(${ox}px, ${oy}px)
    rotateX(${rx}deg) rotateY(${ry}deg)`
    button.style.boxShadow = `0px ${Math.abs(oy)}px ${Math.abs(oy) /
    radius * 40}px rgba(0,0,0,0,15)`
})